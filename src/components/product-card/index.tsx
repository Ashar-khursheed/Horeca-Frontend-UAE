...
const WHATSAPP_PHONE = "971800467322";

const openWhatsAppProduct = (productName: string, productHref: string, qty: number) => {
  const fullLink = `${SITE_URL}${productHref.startsWith("/") ? productHref : `/${productHref}`}`;
  const text = `Hi, I'm interested in this product:\n${productName}\nQuantity: ${qty}\n${fullLink}`;
  const url = `https://api.whatsapp.com/send/?phone=${WHATSAPP_PHONE}&text=${encodeURIComponent(text)}&type=phone_number&app_absent=0`;
      ...
      <div className="relative bg-white">
        ...
        <button
          ...
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            openWhatsAppProduct(name, productLink, minQty || 1);
          }}
          aria-label="Chat on WhatsApp"
          className="absolute top-2.5 right-[50px] z-10 w-[38px] h-[38px] bg-white rounded-full border border-gray-200 shadow-sm flex items-center justify-center hover:bg-gray-50 transition-colors duration-200"
        >
          <svg
            width="24px"
            height="24px"
      ...
      <div className="px-3 pb-3 pt-2 md:px-4 md:pb-4 md:pt-2 flex flex-col flex-1 border-t border-gray-100">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2 flex-wrap leading-none">
                ...
                <div className="flex items-baseline xl:text-[22px]  lg:text-[18px] md:text-base  gap-[1px]">
                  ...
                  <span
                    className={`text-[14px] font-bold leading-none ${hasSale ? "text-[#186737]" : "text-gray-900"
                      }`}
                  >
                    .{priceDec}
                  </span>
        ...
        */}
        ...
        {/* <button
          ...
          //   setQuoteAdded(true);
          // }}
          className={`w-full mt-2 3xl:h-[44px] 2xl:h-[36px] bg-green-700 md:h-[34px] h-[29px] rounded-[4px] 3xl:text-[14px] 2xl:text-[15px] text-[10px] font-semibold flex items-center justify-center gap-1.5 text-white transition-all ${
      ...
      >
       ...
       Whatsapp
        ...
        <button
          ...
          className={`w-full mt-2 3xl:h-[44px] 2xl:h-[36px] md:h-[34px] h-[29px] rounded-[4px] 3xl:text-[14px] 2xl:text-[14px] text-[10px] font-semibold flex items-center justify-center gap-1.5 text-white transition-all ${inQuoteList || quoteAdded
              ? "bg-[#8B1515]"
              : "hover:brightness-110 hover:shadow-md"
            }`}
          style={
            inQuoteList || quoteAdded
              ...
              : {
                background: "#b81414",
                // backgroundImage:
                //   "linear-gradient(201deg,rgba(253, 29, 29, 1) 56%, rgba(252, 176, 69, 1) 93%)",
              }
          }
        >
          {inQuoteList || quoteAdded ? (
