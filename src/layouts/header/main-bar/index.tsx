...
export default function NavigationStatic({
  ...
  const liveDefault = useSelector(
    (s: RootState) =>
      s.customerAddress.addresses.find((a) => a.is_default) ?? null,
  );
  ...
  const wishlistApiCount = useSelector(
    (s: RootState) => s.customerCounts.wishlist_count,
  );
  const cartApiQtySum = useSelector(
    (s: RootState) => s.customerCounts.cart_quantity_sum,
  );
  const wishlistLocalCount = useSelector(
    (s: RootState) => s.wishlist.ids.length,
  );
            ...
            <div className="flex items-center justify-between gap-3 lg:gap-4 xl:gap-3 2xl:gap-5 h-[68px]">
              ...
              >
                ...
                >
                  ...
                  <ChevronDown
                    size={12}
                    className={`text-gray-400 transition-transform duration-200 ${showContact ? "rotate-180 text-[#186737]" : ""}`}
                  />
                </button>

                {showContact && (
                  <div
                    className="absolute top-full right-0 mt-3 w-70 bg-white rounded-xl z-50 overflow-hidden"
                    style={{
                      boxShadow:
                    ...
                    <div className="flex  border-gray-100s  w-[80%] mx-auto ">
                      ...
                      >
                        <div className="w-9 h-9 rounded-full bg-[#186737]/10 flex items-center justify-center group-hover:bg-[#186737] transition-colors">
                          <Phone
                            size={15}
                            className="text-[#186737] group-hover:text-white transition-colors"
                          />
                        </div>
      ...
      Sign in
              ...
              <div className="flex xl:hidden items-center gap-1">
                ...
                <Link href={"/cart"}>
                  <button
                    aria-label="Cart"
                    className="relative w-9 h-9 sm:flex hidden  items-center justify-center rounded-[7px] text-gray-600 hover:bg-gray-100 transition-colors"
                  >
