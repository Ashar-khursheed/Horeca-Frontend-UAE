...
//   // Pricing from saved cart summary (localStorage)`n//   const baseSubtotal = cartSummary?.subTotal ?? 0;
...
//   // Sync updated summary back to localStorage whenever fees / discount change`n//   useEffect(() => {
...
export default function CheckoutPage() {
  ...
  // Derived state`n  const hasAddress =
  ...
  // Pricing`n  const baseSubtotal = cartSummary?.subTotal ?? 0;
  const baseShipping = cartSummary?.totalShippingCharges ?? 0;
  const ratePercent = cartSummary?.taxRatePercentage ?? 0;
  const taxRate = ratePercent / 100;
  const taxable = baseSubtotal - discount;
  ...
  // Sync summary to localStorage`n  useEffect(() => {
    if (!cartSummary) return;
    try {
      const additionalFees = liftFee + resFee + insideFee;
      const updated = {
  ...
  // Place Order handler`n  // const handlePlaceOrder = async () => {
  //   setOrderError(null)
  ...
  //   try {
  //     // Tokenize (Square card details lo)`n  //     let token: any
  //     try {
  ...
  //     // Idempotency key generate karo (fresh, no localStorage)`n  //     const idempotencyKey = crypto.randomUUID()

  //     // Payment API call`n  //     const localTotal = (() => {
  ...
  const handlePlaceOrder = async () => {
    ...
    // STEP 2:`n    if (!paymentHandleRef.current) {
      setOrderError(
    ...
    try {
      ...
      // STEP 4:`n      const idempotencyKey = crypto.randomUUID();

      // STEP 5:`n      const localSummary = (() => {
        try {
      ...
      // STEP 6:`n      const paymentRes = (await makeApiRequest(apiUrls?.SQUARE_PAYMENT, {
        data: {
          source_id: token,
          amount,
          idempotency_key: idempotencyKey,
      ...
      // STEP 7:`n      const defaultAddr = getDefaultAddressCache();
      const user = (() => {
        try {
      ...
      // // STEP 8:`n      // const orderRes = (await makeApiRequest(apiUrls?.PLACE_ORDER, {
      //   data: {
      //     customer_id: user?.id,
      ...
      // STEP 8:`nconst orderRes = (await makeApiRequest(apiUrls?.PLACE_ORDER, {
  data: {
    customer_id: user?.id,
    customer_address_id: defaultAddr?.id,
    tax_percentage: taxPercent,
...
try {
  ...
  console.warn("Order detail fetch failed (non-blocking):", detailErr);
      ...
      // STEP 11:`n      let fullOrder = orderData;
      try {
        const detailRes = (await makeApiRequest(
  ...
  // Shared blocks (used in both mobile & desktop)`n  const contactBlock = (
    <div className="mb-7">
...
// }
  ...
  return (
    <>
      ...
      {/* â•â•â•â•â•â•â•â•â•â•â•â•â•â• MOBILE LAYOUT  -  step-based (hidden on lg+) â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      ...
      {/* â•â•â•â•â•â•â•â•â•â•â•â•â•â• DESKTOP LAYOUT  -  2-column (hidden on mobile) â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
